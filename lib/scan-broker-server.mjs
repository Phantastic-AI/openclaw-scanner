...
export function createScanBrokerServer({
  socketPath,
  logPath,
  logger = console,
  handlers = {},
    ...
    socket.on("end", async () => {
      ...
      try {
        const firstLine = buffer
          ...
          .find(Boolean);
        if (!firstLine) {
          response = createErrorResponse("empty_request", "empty broker request");
        } else {
          ...
          if (request.op === "status") {
            ...
            response = createErrorResponse("unsupported_operation", "unsupported broker operation");
          }
        }
      } catch (error) {
        response = createErrorResponse("internal_error", error);
        logger.warn?.(`[openclaw-sec] request handling failed: ${truncateMessage(error)}`);
      }
      ...
      if (request?.op && request.op !== "status") {
        try {
          ...
          logger.warn?.(`[openclaw-sec] failed to append broker log: ${truncateMessage(error)}`);
        }
      }
    });
  });
